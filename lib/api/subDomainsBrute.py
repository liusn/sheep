...
import optparse
import random
from lib.utils.consle_width import getTerminalSize
...
class SubNameBrute:
    def __init__(self, target, options):
        ...
        if options["output"]:
            ...
            fpath = os.path.join(paths.OUTPUT_PATH, "subdomain")
            if not os.path.exists(fpath):
                os.mkdir(fpath)
            outFileName = target + '.txt' if not options["full_scan"] else target + '_full.txt'
            outfile = os.path.join(fpath, outFileName)
    ...
    def _load_dns_servers(self):
        ...
        with open(paths.DNS_SERVERS) as f:
            for line in f:
                server = line.strip()
                while True:
    ...
    def _load_sub_names(self):
        ...
        if self.options["full_scan"]:
            _file = paths.SUBNAMES_FULL
        else:
            _file = paths.SUBNAMES
        sub_names = set()
        lst_subs = []
    ...
    def _load_next_sub(self):
        ...
        next_subs = []
        _file = paths.NEXT_SUB if not self.options["full_scan"] else paths.NEXT_SUB_FULL
        with open(_file) as f:
            for line in f:
                sub = line.strip()
...
if __name__ == '__main__':
    ...
    if len(args) < 1:
        ...
        sys.exit(0)
    args = {}
    args["full_scan"] = options.full_scan
    args["i"] = options.i
    args["output"] = options.output
    d = SubNameBrute(target=args[0], options=args)
    d.run()
    d.outfile.flush()
    d.outfile.close()
...
def run(args):
    targ = args["domain"]
    d = SubNameBrute(target = targ, options = args)
    d.run()
    d.outfile.flush()
    d.outfile.close()
    return
